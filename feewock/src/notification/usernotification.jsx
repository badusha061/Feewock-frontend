import React, { useEffect, useState, useReducer } from 'react'
import Layouts from '../layouts/Layouts'
import Spinner from '../utils/Spinner'
import useAxios from '../AxiosConfig/Axios'
import moment from 'moment';
import { Toaster } from 'react-hot-toast';
import { toast } from 'react-hot-toast';
import { AiTwotoneDelete } from "react-icons/ai";
import { IoIosArrowForward } from "react-icons/io";
import { Navigate, useNavigate } from 'react-router-dom';

function Usernotification() {
  const navigate = useNavigate()
  const [notification, setNotification] = useState([])
  const [isLoading, setIsLoading] = useState(true)
  const axiosInstance = useAxios()
  const userDetails = JSON.parse(localStorage.getItem('userDetails'));
  const [reducer, forceUpdate] = useReducer(x => x + 1, 0)
  const [modal, setModal] = useState(false)

  let UserId
  if (userDetails && userDetails.id) {
    UserId = userDetails.id
  }

  useEffect(() => {
    GetNotification()
  }, [UserId, reducer])

  const GetNotification = async () => {
    const response = await axiosInstance.get(`/booking/usernotification/${UserId}/`)
    if (response.status === 200) {
      setNotification(response.data.data)
      setIsLoading(false)
    }
  }
  if (isLoading) {
    return <Spinner />
  }
  console.log(notification);

  {
    notification.map((data, index) => {
      console.log(data.action.action);
    })
  }

  const handleClick = async (e) => {
    e.preventDefault()
    const response = await axiosInstance.delete(`/booking/deleteuser/${UserId}/`)
    if (response.status === 200) {
      toast.success("Successfully Removed all Notification")
      handleClose()
      return true
    }
  }
  const handleModal = () => {
    setModal(true)
  }
  const handleClose = () => {
    setModal(false)
    forceUpdate()
  }

  const handleNavigate = () => {
    navigate('/bookinglist', { state: UserId })
  }
  return (

    <Layouts>

      {notification.length > 0 ? (


        <div className="lg:w-2/5 sm:w-3/5 w-11/12 bg-gray-100 dark:bg-gray-800 rounded-xl mx-auto border p-10 shadow-sm">
          <div className="inline-flex items-center justify-between w-full">
            <h3 className="font-bold text-xl sm:text-2xl text-gray-800 dark:text-white">Notifications</h3>
          </div>
          <p className="mt-8 font-medium text-gray-500 text-sm sm:text-base dark:text-white">Accept</p>
          {notification.map((data, index) => (
            <>
              {data.action.action === 'accepted' ? (

                <div onClick={handleNavigate} className="mt-2 px-6 py-4 cursor-pointer  bg-white rounded-lg shadow w-full">
                  <div className=" inline-flex items-center justify-between w-full">

                    <div className="inline-flex items-center">
                      <img
                        src="data:image/png;base64,"
                        alt="Training Icon" className="w-6 h-6 mr-3" />
                      <h3 className="font-bold text-base text-green-600">Your Service is Accepted</h3>
                    </div>
                    <IoIosArrowForward  size={30} />
                  </div>
                  <p className="mt-1 text-sm">
                    {data.action.appointment.name}, {data.action.appointment.location} 
                  </p>
                  <p className="text-xs text-gray-500">
                      {moment.utc(data.created_at).local().startOf('seconds').fromNow()}
                  </p>
                </div>

              ) : null}

            </>
          ))}


          <p className="mt-8 font-medium text-gray-500 dark:text-white text-sm sm:text-base">Reject</p>
          {notification.map((data, index) => (
            <>
              {data.action.action === 'rejected' ? (

                <div onClick={handleNavigate}  className="mt-2 px-6 cursor-pointer py-4 bg-white rounded-lg shadow w-full">
                  <div className=" inline-flex items-center justify-between w-full">
                    <div className="inline-flex items-center">
                      <img
                        src="https://www.shutterstock.com/image-vector/rejected-icon-vector-symbol-illustration-260nw-1753295894.jpg"
                        alt="Form Icon" className="w-6 h-6 mr-3" />
                      <h3 className="font-bold text-base text-red-600">Your Service is Rejected</h3>
                    </div>
                    <IoIosArrowForward size={30} />
                  </div>
                  <p className="mt-1 text-sm">
                    Reason: {data.action.comment}
                  </p>
                  <p className="text-xs text-gray-500">
                      {moment.utc(data.created_at).local().startOf('seconds').fromNow()}
                    </p>
                </div>
              ) : null}

            </>
          ))}

          <button
            onClick={handleModal}
            className="inline-flex text-sm bg-white justify-center px-4 py-2 mt-12 w-full text-red-500 items-center rounded font-medium
     shadow border focus:outline-none transform active:scale-75 transition-transform duration-700 hover:bg-red-500
      hover:text-white hover:-translate-y-1 hover:scale-110 dark:hover:bg-white dark:text-gray-800 dark:hover:text-gray-800">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-2 sm:mr-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            Clear all notifications
          </button>
        </div>
      ) : (
        <div class="w-full md:w-1/3 mx-auto">
        <div class="flex p-5 rounded-lg shadow bg-white">
        <div>
          <svg class="w-6 h-6 fill-current text-custom-blue" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M12 5.99L19.53 19H4.47L12 5.99M12 2L1 21h22L12 2zm1 14h-2v2h2v-2zm0-6h-2v4h2v-4z"/></svg>
        </div>
        <div class="ml-3">
          <h2 class="font-semibold text-gray-800">No Notification Found</h2>
        </div>
        </div>
      </div>
      )}


      {modal ? (
        <div
          className="fixed inset-0 p-4 flex flex-wrap justify-center items-center w-full h-full z-[1000] before:fixed before:inset-0 before:w-full before:h-full before:bg-[rgba(0,0,0,0.5)] overflow-auto font-[sans-serif]">
          <div onClick={handleClose} className="w-full max-w-md bg-white shadow-lg rounded-md p-6 relative">
            <svg xmlns="http://www.w3.org/2000/svg"
              className="w-3.5 cursor-pointer shrink-0 fill-black hover:fill-red-500 float-right" viewBox="0 0 320.591 320.591">
              <path
                d="M30.391 318.583a30.37 30.37 0 0 1-21.56-7.288c-11.774-11.844-11.774-30.973 0-42.817L266.643 10.665c12.246-11.459 31.462-10.822 42.921 1.424 10.362 11.074 10.966 28.095 1.414 39.875L51.647 311.295a30.366 30.366 0 0 1-21.256 7.288z"
                data-original="#000000"></path>
              <path
                d="M287.9 318.583a30.37 30.37 0 0 1-21.257-8.806L8.83 51.963C-2.078 39.225-.595 20.055 12.143 9.146c11.369-9.736 28.136-9.736 39.504 0l259.331 257.813c12.243 11.462 12.876 30.679 1.414 42.922-.456.487-.927.958-1.414 1.414a30.368 30.368 0 0 1-23.078 7.288z"
                data-original="#000000"></path>
            </svg>
            <div className="my-8 text-center">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-16 fill-red-500 inline" viewBox="0 0 24 24">
                <path
                  d="M19 7a1 1 0 0 0-1 1v11.191A1.92 1.92 0 0 1 15.99 21H8.01A1.92 1.92 0 0 1 6 19.191V8a1 1 0 0 0-2 0v11.191A3.918 3.918 0 0 0 8.01 23h7.98A3.918 3.918 0 0 0 20 19.191V8a1 1 0 0 0-1-1Zm1-3h-4V2a1 1 0 0 0-1-1H9a1 1 0 0 0-1 1v2H4a1 1 0 0 0 0 2h16a1 1 0 0 0 0-2ZM10 4V3h4v1Z"
                  data-original="#000000" />
                <path d="M11 17v-7a1 1 0 0 0-2 0v7a1 1 0 0 0 2 0Zm4 0v-7a1 1 0 0 0-2 0v7a1 1 0 0 0 2 0Z"
                  data-original="#000000" />
              </svg>
              <h4 className="text-xl font-semibold mt-6">Are you sure you want to delete All notifications?</h4>
              <p className="text-sm text-gray-500 mt-4">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed auctor auctor
                arcu,
                at fermentum dui. Maecenas</p>
            </div>
            <div className="flex flex-col space-y-2">
              <button
                onClick={handleClick}
                type="button"
                className="px-6 py-2.5 rounded-md text-white text-sm font-semibold border-none outline-none bg-red-500 hover:bg-red-600 active:bg-red-500">Delete</button>
              <button
                type="button"
                onClick={handleClose}
                className="px-6 py-2.5 rounded-md text-black text-sm font-semibold border-none outline-none bg-gray-200 hover:bg-gray-300 active:bg-gray-200">Cancel</button>
            </div>
          </div>
        </div>
      ) : null}


      <Toaster
        position="top-center"
        reverseOrder={false}
      />



    </Layouts>

  )
}

export default Usernotification
